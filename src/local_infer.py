...
@measure_time
def local_infer(infer_url, data):
    try:
        response = requests.post(infer_url, json=data)
        resp_dict = json.loads(response.text)
        if resp_dict.get("response", None):
            return True, resp_dict["response"]
        else:
            return False, "can not fetch resp"
...
from db.db_mysql import AbstractOldBase


class Menus(AbstractOldBase):
...
if __name__ == "__main__":
    code_snippet_path = "apps/permission/models/menu.py"
    data = {
        "prompt": f"请你阅读以下代码片段，这个代码片段所在文件路径为{code_snippet_path},代码内容为:{sample_code}，请告诉我你分析这段代码的主要作用是？",
        "model": "codestral",
        "stream": False }
    succ, resp = local_infer(url, data)
    print(succ, resp)